...
//
// If reset is true, calls to Next after calling AsRawHeader on i will indicate
// that the iterator is done.
func (i *IPv6PayloadIterator) AsRawHeader(reset bool) IPv6RawPayloadHeader {
	identifier := i.nextHdrIdentifier

	var buf buffer.VectorisedView
	if reset {
		// Since we reset the iterator, we return the buffer.Vectorised as is.
		buf = i.payload
...
func (i *IPv6PayloadIterator) Next() (IPv6PayloadHeader, bool, error) {
	...
	if i.forceRaw {
		return i.AsRawHeader(true /* reset */), false, nil
	}
