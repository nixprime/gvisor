...
func (e *endpoint) HandlePacket(r *stack.Route, pkt stack.PacketBuffer) {
	...
	for firstHeader := true; ; firstHeader = false {
		...
		case header.IPv6FragmentExtHdr:
			...
			// Don't reset the iterator if we have the first fragment because we will
			// use it to validate that the first fragment holds the upper layer
			// header.
			rawPayload := it.AsRawHeader(fragmentOffset != 0 /* reset */)

			if fragmentOffset == 0 {
				// Make sure that the iterator ends with a raw payload as the first
				// fragment should include all headers up to and including any upper
				// layer headers, as per RFC 8200 section 4.5; only upper layer data
				...
				// If lastHdr is of type header.IPv6RawPayloadHeader, then we know the
				// last portion of the IPv6 payload is not a known IPv6 extension
				// header. Note, this does not mean that the last portion is an upper
				// layer header or not an extension header because:
