...
type Stack struct {
	...
	*ports.PortManager
	...
	// hasNUDSupport determines whether or not the stack has support for NUD.
	hasNUDSupport bool
...
func New(opts Options) *Stack {
	...
	_, hasARP := s.linkAddrResolvers[header.ARPProtocolNumber]
	_, hasNDP := s.linkAddrResolvers[header.IPv6ProtocolNumber]
	s.hasNUDSupport = hasARP || hasNDP
...
func (s *Stack) AddStaticNeighbor(nicID tcpip.NICID, addr tcpip.Address, linkAddr tcpip.LinkAddress) *tcpip.Error {
	...
	nic.addStaticNeighbor(addr, linkAddr)
	return nil
}
...
func (s *Stack) RemoveNeighbor(nicID tcpip.NICID, addr tcpip.Address) *tcpip.Error {
	...
	if ok := nic.removeNeighbor(addr); !ok {
