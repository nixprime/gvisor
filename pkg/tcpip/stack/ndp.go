...
// conflict. If dadConflict is true and addr is a SLAAC address, a new address
// will attempt to be generated in response to the DAD conflict if the NIC is
// configured to do so and the prefix has not already reached the maximum number
// of retry attempts.
//
// The NIC that ndp belongs to MUST be locked.
func (ndp *ndpState) stopDuplicateAddressDetection(addr tcpip.AddressWithPrefix, dadConflict bool) *tcpip.Error {
	...
	// If the conflict was for a SLAAC address, attempt to generate a new address
	// for the SLAAC prefix if configured to do so and the IID generation method
	// supports regeneration for DAD conflicts.

	prefix := addr.Subnet()
	state, ok := ndp.slaacPrefixes[prefix]
	// If addr was not generated via SLAAC or the IID generation method does not
	// support regeneration for DAD conflicts, just remove the address and do
	// nothing further.
	if !ok || ndp.nic.stack.opaqueIIDOpts.NICNameFromID == nil {
		return ndp.nic.removePermanentAddressLocked(addr.Address)
	}

	// Have we already reached the maximum generation attempts?
	if state.dadCounter == state.maxDADRetries {
		ndp.invalidateSLAACPrefix(prefix, true /* removeAddr */)
		return nil
	}

	// Remove the original address for prefix that failed DAD.
	state.ref = nil
